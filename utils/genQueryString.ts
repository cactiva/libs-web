...
export const genCounts = (
  ...
  return `${tabs(level)}${table.name}_aggregate${
...
export const genFields = (
  ...
  if (_.get(options, "showArgs", true)) {
    ...
    if (table.options) {
      if (table.options.limit) {
        args.push(`limit: ${table.options.limit}`);
      }
      if (table.options.offset) {
        args.push(`offset: ${table.options.offset}`);
      }
      if (table.options.distinct_on) {
        args.push(`distinct_on: ${table.options.distinct_on}`);
      }
    }
  }
